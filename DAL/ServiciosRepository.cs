...
namespace DAL
{
    public class ServiciosRepository
    {
        ...
        public void Guardar(Servicio Servicio)
        {
            ...
            {
                command.CommandText = @"Insert Into mascotas (CodigoServicio,NombreServicio,PrecioServicio) 
...

        {
            using (var command = _connection.CreateCommand())
            {
                command.CommandText = "update Servicios set  NombreServicio=@NombreServicio, PrecioServicio=@PrecioServicio";
                command.Parameters.AddWithValue("@NombreServicio", Servicio.NombreServicio);                
                command.Parameters.AddWithValue("@PrecioServicio", Servicio.PrecioServicio);
                var filas = command.ExecuteNonQuery();
            }
